...
// Stop filler explicitly on all players
app.post('/api/filler/stop', (_req, res) => {
    try {
        broadcast({ type: 'stop_filler_music' });
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ success: false, error: String(err) });
    }
...
app.post('/api/filler/volume', (req, res) => {
    try {
        const vol = (req.body && typeof req.body === 'object' && 'volume' in req.body) ? Number((req.body as { volume: number }).volume) : NaN;
        if (Number.isNaN(vol)) return res.status(400).json({ success: false, error: 'volume required' });
...
app.get('/api/songs', (req, res) => {
    ...
    // Return raw filenames matched by case-insensitive substring
    const results = searchSongs(query).map(s => ({ id: s.id, artist: '', title: s.fileName, fileName: s.fileName }));
    res.json(results);
});
...
app.get(/^\/api\/media\/(.+)/, (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Next fallback: check filler music directory
            try {
